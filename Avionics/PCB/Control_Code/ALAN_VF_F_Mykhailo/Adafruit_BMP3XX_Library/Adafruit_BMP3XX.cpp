...
bool Adafruit_BMP3XX::startNormalModeFast(uint8_t pressure_os,
    uint8_t temp_os,
    uint8_t iir_filter,
    uint8_t odr,
    bool enable_temp) {
    ...
    if (enable_temp) {
        ...
        the_sensor.settings.temp_en = BMP3_DISABLE;
    }
    settings_sel |= BMP3_SEL_TEMP_EN;

    the_sensor.settings.odr_filter.press_os = pressure_os;
    settings_sel |= BMP3_SEL_PRESS_OS;

    the_sensor.settings.odr_filter.temp_os = temp_os;
    settings_sel |= BMP3_SEL_TEMP_OS;

    the_sensor.settings.odr_filter.iir_filter = iir_filter;
    settings_sel |= BMP3_SEL_IIR_FILTER;
...
bool Adafruit_BMP3XX::readNormalModeData(bool check_data_ready,
    bool read_temp) {
    g_i2c_dev = i2c_dev;
    g_spi_dev = spi_dev;

    if (check_data_ready) {
        int8_t status_rslt = bmp3_get_status(&the_sensor);
        if (status_rslt != BMP3_OK) {
            return false;
        }

        // For flight barometer, pressure-ready is enough.
        // Do not block waiting for temperature-ready.
    ...
    uint8_t sensor_comp = read_temp ? BMP3_ALL : BMP3_PRESS;
