...
void printPacketHex(const char* tag, uint8_t* buf, uint8_t len) {
  Serial.print(tag);
  Serial.print(" len=");
  Serial.print(len);
  Serial.print(" data=");
...
void sendSetEngineTimingCommand(uint32_t auto_close_delay_ms,
                                uint32_t min_engine_on_time_ms) {
  uint8_t p[24];
  uint8_t i = 0;

  packU8(p, i, MSG_CLASS_COMMAND);
  packU8(p, i, CMD_SET_ENGINE_TIMING);
  ...
  packU32(p, i, auto_close_delay_ms);
  packU32(p, i, min_engine_on_time_ms);
...
void decodeFastTelemetry(const uint8_t* payload, uint8_t len, int16_t rssi) {
  // Old ALAN fast payload was 31 bytes.
  // New ALAN fast payload is 35 bytes:
  //   + current_max_altitude_10m uint16
  //   + module_status_flags uint8
  //   + makes_sense_flags uint8
  // inserted before timeline_ms.
  if (len < 31) {
  ...
  // Default values for older 31-byte packets.
  uint16_t current_max_altitude_10m = 0;
  uint8_t module_status_flags = 0;
  uint8_t makes_sense_flags = 0;

  // New ALAN_VF_2 packet: these three fields are before timeline_ms.
  
    current_max_altitude_10m = readU16(payload + i); i += 2;
    module_status_flags = payload[i++];
    makes_sense_flags = payload[i++];
  ...
  Serial.print("\"current_max_altitude_m\":");
  Serial.print((uint16_t)(current_max_altitude_10m * 10));
  Serial.print(',');

  Serial.print("\"current_max_altitude_10m\":");
  Serial.print(current_max_altitude_10m);
  Serial.print(',');

  Serial.print("\"module_status_flags\":");
  Serial.print(module_status_flags);
  Serial.print(',');

  Serial.print("\"makes_sense_flags\":");
  Serial.print(makes_sense_flags);
  Serial.print(',');
...
void decodeGpsTelemetry(const uint8_t* payload, uint8_t len, int16_t rssi) {
  // Old GPS payload was 17 bytes.
  // New ALAN GPS payload is 23 bytes:
  //   + DS1307 time: yy, month, day, hour, minute, second.
  if (len < 17) return;
  ...
  if (len >= 23) {
    ...
    rtc_day    = payload[i++];
    rtc_hour   = payload[i++];
    rtc_minute = payload[i++];
    rtc_second = payload[i++];
    has_rtc_time = 1;
  }

  uint16_t main_auto_close_10ms = 0;
  uint16_t min_engine_on_10ms = 0;

  //uint32_t main_valve_auto_close_delay_ms = 0;
  uint32_t max_engine_on_time_ms = 0;
  uint32_t min_engine_on_time_ms = 0;

  uint8_t has_engine_timing = 0;

  if (len >= 27) {
    main_auto_close_10ms = readU16(payload + i); i += 2;
    min_engine_on_10ms   = readU16(payload + i); i += 2;

    max_engine_on_time_ms          = (uint32_t)main_auto_close_10ms * 10UL;
    //max_engine_on_time_ms        = main_valve_auto_close_delay_ms;
    min_engine_on_time_ms          = (uint32_t)min_engine_on_10ms * 10UL;
  ...
  Serial.print("\"has_rtc_time\":"); Serial.print(has_rtc_time); Serial.print(',');

  Serial.print("\"rtc_year\":"); Serial.print(rtc_year); Serial.print(',');
  Serial.print("\"rtc_month\":"); Serial.print(rtc_month); Serial.print(',');
  Serial.print("\"rtc_day\":"); Serial.print(rtc_day); Serial.print(',');
  Serial.print("\"rtc_hour\":"); Serial.print(rtc_hour); Serial.print(',');
  Serial.print("\"rtc_minute\":"); Serial.print(rtc_minute); Serial.print(',');
  Serial.print("\"rtc_second\":"); Serial.print(rtc_second); Serial.print(',');

  Serial.print("\"rtc_time\":\"");
  printTwoDigit(rtc_year);
  Serial.print('-');
  printTwoDigit(rtc_month);
  Serial.print('-');
  printTwoDigit(rtc_day);
  Serial.print(' ');
  printTwoDigit(rtc_hour);
  Serial.print(':');
  printTwoDigit(rtc_minute);
  Serial.print(':');
  printTwoDigit(rtc_second);
  Serial.print("\",");

  // Serial.print("\"has_engine_timing\":");
  // Serial.print(has_engine_timing);
  // Serial.print(',');

  // Serial.print("\"main_valve_auto_close_delay_ms\":");
  // Serial.print(main_valve_auto_close_delay_ms);
  // Serial.print(',');

  Serial.print("\"max_engine_on_time_ms\":");
  Serial.print(max_engine_on_time_ms);
  Serial.print(',');

  Serial.print("\"min_engine_on_time_ms\":");
  Serial.print(min_engine_on_time_ms);
  Serial.print(',');
...
void handleSerialCommand(char* line) {
  ...
  if (strncmp(line, "SET_ENGINE_TIMING ", 18) == 0) {
  unsigned long max_engine_on_ms;
  unsigned long min_engine_on_ms;

    if (sscanf(line + 18, "%lu %lu", &max_engine_on_ms, &min_engine_on_ms) == 2) {
      sendSetEngineTimingCommand((uint32_t)max_engine_on_ms,
                                (uint32_t)min_engine_on_ms);
    } else {
      Serial.println(F("{\"type\":\"cmd_error\",\"reason\":\"bad_SET_ENGINE_TIMING_format\"}"));
    } 
