...
* V3.1 expanded the pressure system from a single pressure sensor to all four pressure sensors using the pins defined at the top of the file.
* Radio telemetry was updated so the board could send back both the pressure data and the valve/state information using the selected avionics message IDs.
* New initialization and read functions were added for the LSM6DSOX and ADXL375 sensors, although their data was not yet included in radio telemetry.
* The debug system was kept and extended by preserving the menu, local status, and existing test functions while adding a radio check message such as `"PCB online"`.
* Module startup reporting was improved so each initialized subsystem printed an `"online"` message with its module name.
* SD card logging was added so the system could save valve states, all four pressure readings, and the LSM6DSOX and ADXL375 data into a structured log file.
* The overall code organization and section structure from V3 were kept the same while adding the new functionality.
...
//================ DYNAMIC VARIABLES ================
  ...
  // Other state
    ...
    bool send_check_message_requested = false;
    char check_message_text[24] = "PCB online";
...
//================ MAIN LOOP ================
  void loop() {
    ...
    if (send_check_message_requested) {
      send_check_message();
      send_check_message_requested = false;
    } else {
      prep_telemetry();
      send_RFM();
    }
...
//================ TELEMETRY CODE ================
  uint32_t compute_valve_status_array() {
    // 0 = none open, 1 = relief open, 2 = main open, 3 = both open
    uint32_t status = 0;

    if (RELIEF_VALVE.state) status |= 0x01;
    if (MAIN_VALVE.state)   status |= 0x02;
...
//================ RADIO CODE ================
  ...
  void send_check_message() {
    if (!RFM9X_FAIL && TX_enabled) {
      rf95.send((uint8_t*)check_message_text, strlen(check_message_text) + 1);
      RFM9x_last_time = micros();
      Serial.println("radio check message sent");
    }
  }
...
//================ SD CARD CODE ================
  void init_SD() {
    ...
    if (!SD.exists("log.csv")) {
      ...
      if (logFile) {
        logFile.println("time_ms,main_state,relief_state,dump_state,pressure_1,pressure_2,pressure_3,pressure_4,lsm_ax,lsm_ay,lsm_az,lsm_gx,lsm_gy,lsm_gz,lsm_temp,adxl_ax,adxl_ay,adxl_az");
        logFile.close();
      }
    }
...
//================ DEBUG & MANUAL CONTROL ================
  void debug_menu() {
    ...
    Serial.println("p = read and print all pressures");
    Serial.println("i = print IMU values");
    Serial.println("t = build telemetry and print raw packet");
    Serial.println("o = send check message: PCB online");
  ...
  void debug_serial() {
    ...
    switch (c) {
      ...
      case 'o':
        send_check_message_requested = true;
        Serial.println("Check message requested");
        break;
