/*
  ...
  to change
    !slight ARM changes
      !No timer
    !slight Ignition Sequence changes
      !28 seconds goes from -28 to +0 at +0 main valve open stage 4 count continues
      GUI graph starts at Ignition sequence so -8
    !Real Desired Apogee = Desired apogee + initial SL alt
    !RX TX GPS
    !Max alt reached radio pack Add to radio constant send Fail flags (1 byte) and add current max alt reach (int 2 bytes from 0 to 512 with 10m increament so from 0 to 5120 m) 
      Add constant GPS send after relief open remove some other data from radio package (Vel, angle, )
    Con check check
    SD log change for new important

    New BME

    GUI:
    !Move fail and makes sence modules to  constant send
    !max alt window
    GPS show map
    Command send/receive log count
    !Last update time on all data
...
// ======================= HARDWARE CONNECTIONS ====================
  ...
  // ---------------- PCB input pins: pressure transducers ------------
    ...
    const uint8_t PRESSURE_TRANSDUCER_2_pin = 20;
...
// ============================ CONSTANTS ==========================
  ...
  // Pressure calibration placeholders from old code style.
    // const float PRESSURE_1_SLOPE  = 1.0f; delete
    // const float PRESSURE_1_OFFSET = 0.0f; delete
    const float PRESSURE_2_SLOPE  = 1.0f;
    const float PRESSURE_2_OFFSET = 0.0f;
    // const float PRESSURE_3_SLOPE  = 1.0f; delete
    // const float PRESSURE_3_OFFSET = 0.0f; delete
    // const float PRESSURE_4_SLOPE  = 1.0f; delete
    // const float PRESSURE_4_OFFSET = 0.0f; delete
...
// ======================== DYNAMIC VARIABLES ======================
  ...
  // ---------------- Old ignition-system state -----------------------
    ...
    const int8_t IGNITION_COUNTDOWN_START_S = -28;
...
// MISSLE KNOWS WHERE IT IS & MISSLE KNOWS WHERE IT WILL BE (ORACLE) 
  void set_to_start_location() {
    ...
    initial_SL_altitude_m = global_location.z;
  ...
  void ORACLE_update() { // will determine location and vel vectors for next simulation
    LSM_to_PCB_switch_over(); // accelem orientation to rocket orientation
  ...
  // red flags
    void update_sensor_makes_sense() {
      ...
      if (BME680_FAIL) {
        ...
        if (!bme_bad) {
          previous_BME_altitude_for_sense_m = BME680_altitude_m;
...
// ====================== DEBUG / LOCAL TEST SECTION ===============
  ...
  void streams() {
    ...
    if (bme_stream_enabled) {
      ...
      // Serial.print(" | Hz = "); delete
      // Serial.println(BME680_update_hz, 2);  delete
    }
