...
// ============================ CONSTANTS ==========================
  ...
  // Radio frequency from YS_Alan_V.4.1
  ...
  // Old ignition-system reference values.
    ...
    const uint32_t MAIN_VALVE_OPEN_AFTER_START_MS  = 3000;
...
// ======================== DYNAMIC VARIABLES ======================
  ...
  // ---------------- Old ignition-system state -----------------------
    ...
    const int8_t IGNITION_COUNTDOWN_START_S = -5;
    const int8_t ARM_COUNTDOWN_START_S = -5;
    ...
    // This follows old ignition timing idea.
      const uint32_t ARM_SEQUENCE_DURATION_MS = 5000;  
    ...
    int32_t compute_radio_timeline_ms() {
      ...
      if (stage == STAGE_LAUNCH_IGNITION_SEQUENCE) {
        if (main_valve_ignition_hold_active &&
          ...
          if (elapsed_ms < MAIN_VALVE_OPEN_AFTER_START_MS) {
            return -(int32_t)(MAIN_VALVE_OPEN_AFTER_START_MS - elapsed_ms);
          }
      ...
      if (stage == STAGE_ENGINE_WORKING ||
        ...
        if (flight_start_time_ms > 0) {
          return (int32_t)(now_ms - flight_start_time_ms);
        }
