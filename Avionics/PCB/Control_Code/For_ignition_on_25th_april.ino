...
//================ IGNITION MAIN VALVE HOLD ================
  ...
  void update_main_valve_ignition_hold() {
    ...
    if (millis() - main_valve_ignition_start_time_ms >= MAIN_VALVE_OPEN_AFTER_START_MS+MAIN_VALVE_AUTO_CLOSE_DELAY_MS) {
      ...
      // return system to "not armed" state
...
//================ ANALOG / PRESSURE CODE ================
  ...
  void read_pressure() {
    if (micros() - Pressure_last_time > Pres_rate) {
      ...
      pressure_3_output = 2.5f * 1000.0f * voltage - 662.5f;
    ...
    if (!RFM9X_FAIL && rf95.available()) {
      ...
      if (rf95.recv(buf, &len)) {
        ...
        if (recv_ready) {
          ...
          // Ground Station expects "ACK1" from AV1
          ...
          switch (msg_id) {
            ...
            case 0x03: // ignition sequence start
              if (avionics_armed) {
                ...
                // lock radio close command until timer ends
            ...
            case 0x05: // close main valve
              ...
              if (from_radio && !main_valve_can_close_by_radio) {
                ...
                // once main is closed after ignition, return to unarmed state
                avionics_armed = false;
                ignition_sequence_started = false;
...
//================ DEBUG & MANUAL CONTROL ================
  ...
  void print_local_status() {
    ...
    Serial.print("AVIONICS armed            = ");
    Serial.println(avionics_armed);

    Serial.print("Ignition started          = ");
    Serial.println(ignition_sequence_started);

    Serial.print("MAIN close by radio       = ");
    Serial.println(main_valve_can_close_by_radio);

    Serial.print("MAIN ignition hold active = ");
    Serial.println(main_valve_ignition_hold_active);

    Serial.print("MAIN auto-close delay ms  = ");
    Serial.println(MAIN_VALVE_AUTO_CLOSE_DELAY_MS);
  ...
  void inject_local_command(uint8_t msg_id) {
    ...
    perform_command(0x01, msg_id, false);
    print_local_status();
  }
  ...
  void print_ignition_control_status() {
    ...
    if (main_valve_ignition_hold_active) {
      ...
      if (elapsed_ms < MAIN_VALVE_AUTO_CLOSE_DELAY_MS) {
        remaining_ms = MAIN_VALVE_AUTO_CLOSE_DELAY_MS - elapsed_ms;
      }

      Serial.print("MAIN hold elapsed ms      = ");
      Serial.println(elapsed_ms);

      Serial.print("MAIN hold remaining ms    = ");
